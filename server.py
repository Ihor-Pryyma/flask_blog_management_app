...
def delete_user(user_id):
    ...
    db.session.commit()
    return jsonify({"message": f"User {user.name }was deleted"}), 204
...
def delete_last_10_blog_posts():
    ...
    for _ in range(10):
        post_to_delete = stack.pop()
        db.session.delete(post_to_delete.data)
        db.session.commit()
    return jsonify({"message": "Last 10 blog posts were successfully deleted"}), 204
